...
def home():
    # provides signup/in, 
    # search of identifiers collection, 
    # search of regex collection, 
    # rate of regex, recent usage, 
    # popular regex by use and rating, 
    # submit of regex or identifier, 
    # some instructions, general info
    pass
...
def content(path):
    # show content pages, such as FAQ etc
...
class SubmitView(MethodView):
    ...
    May want submission restricted to signed up/in users'''
    def get(self):
        if not auth.collection.create(current_user, None):
    ...
    def post(self):
        ...
        try:
            ...
            flash('Successfully received %s' % type)
